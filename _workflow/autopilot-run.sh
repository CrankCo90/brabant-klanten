...
subprocess.run(["bash","-lc","cd /root/klanten && OUTREACH_DATA=/root/outreach-data OUTREACH_CAP=%s /usr/bin/python3 _workflow/outreach/send-outreach.py >> /root/outreach-data/send.log 2>&1"%str(ap.get("cap",20))])
PY
