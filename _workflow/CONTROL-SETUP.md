...
```bash
( crontab -l 2>/dev/null | grep -v 'DAGELIJKSE-RONDE'; echo '0 9 * * * /bin/bash -lc "cd /root/klanten && claude -p \"$(cat _workflow/DAGELIJKSE-RONDE.md)\" --permission-mode acceptEdits >> /root/outreach-data/ochtend.log 2>&1"' ) | crontab -
```
